...
export default function ManajemenAkunPage() {
  ...
  return <div className='space-y-3'><div className='card flex gap-2'><Input placeholder='Cari pengguna' value={q} onChange={(e) => setQ(e.target.value)} /><Button onClick={() => setOpen(true)}>Create</Button></div>
    <Table head={['Nama','Email','Role','createdOn','Aksi']}>{(data?.data || []).map((u: any) => <tr key={u.userId}><td className='px-3 py-2'>{u.username}</td><td>{u.email}</td><td>{u.role}</td><td>{String(u.createdOn || '')}</td><td className='space-x-2'><Button className='bg-red-600' onClick={async () => { if (confirm('Yakin untuk menghapus?')) { await http(`/api/users/${u.userId}/status`, { method: 'PATCH' }); refetch(); } }}>Delete</Button></td></tr>)}</Table>
