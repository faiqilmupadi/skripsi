...
export async function GET(req: Request) {
  ...
  const rows = await query('SELECT userId, username, email, role, createdOn FROM users WHERE (username LIKE ? OR email LIKE ?) LIMIT 10 OFFSET ?', [`%${q}%`, `%${q}%`, offset]);
...
export async function POST(req: Request) {
  ...
  await query('INSERT INTO users(username,email,password,role,createdOn,lastChange) VALUES(?,?,?,?,NOW(),NOW())', [username, email, await bcrypt.hash(password, 10), role]);
